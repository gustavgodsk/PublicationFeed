...
<a href={model.href}>
    <Card
        class={`hover:-translate-y-[2px] ${model.statusToneClass} h-full transition-all duration-200`}
    >
        <CardContent class="flex h-full flex-col gap-4 ">
            ...
            <p
                class="[display:-webkit-box] overflow-hidden text-sm text-muted-foreground [-webkit-box-orient:vertical] [-webkit-line-clamp:2]"
            >
                {model.impact}
            </p>

            <div class="mt-auto space-y-3">
                <div class="grid gap-3 rounded-lg md:grid-cols-3">
                    <div class="space-y-2">
                        <span class="text-xs font-semibold">For</span>
                        <div class="flex flex-wrap gap-1">
                            {#if model.votesForCount === 0}
                                <span class="text-xs text-muted-foreground">Ingen</span>
                            {:else}
                                ...
                                {#if model.votesForCount > model.votesFor.length}
                                    <span class="text-xs text-muted-foreground"
                                        >+{model.votesForCount - model.votesFor.length}</span
                                    >
                                {/if}
                            {/if}
                        </div>
                    ...
                    <div class="space-y-2">
                        <span class="text-xs font-semibold">Imod</span>
                        <div class="flex flex-wrap gap-1">
                            ...
                            {:else}
                                ...
                                {#if model.votesAgainstCount > model.votesAgainst.length}
                                    <span class="text-xs text-muted-foreground"
                                        >+{model.votesAgainstCount -
                                            model.votesAgainst.length}</span
                                    >
                                {/if}
                            {/if}
                        </div>
                    ...
                    <div class="space-y-2">
                        <span class="text-xs font-semibold">Undlod</span>
                        <div class="flex flex-wrap gap-1">
                            ...
                            {:else}
                                ...
                                {#if model.votesAbstainCount > model.votesAbstain.length}
                                    <span class="text-xs text-muted-foreground"
                                        >+{model.votesAbstainCount -
                                            model.votesAbstain.length}</span
                                    >
                                {/if}
                            {/if}
                        </div>
